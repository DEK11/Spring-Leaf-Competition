...
#handling the VAR_0200 column
new = pd.read_csv('new.csv', header=0)
test.VAR_0200 = new.VAR_0200
...
#df["ID"] = testid
#df["target"] = output
#df.to_csv('RandomTree.csv', index = False)

matrix = xgb.DMatrix(train.values, label = target)
train = []
...
num_rounds = 3000
model = xgb.train(params, matrix, num_rounds)
matrix = xgb.DMatrix(test.values)
test = []
...
df1["ID"] = testid
df1["target"] = pred
df1.to_csv('xgboost.csv', index = False)
